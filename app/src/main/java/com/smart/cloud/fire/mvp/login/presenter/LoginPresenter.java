        ...
        addSubscription(observable,new SubscriberCallBack<>(new ApiCallback<LoginModel>() {
            ...
            public void onSuccess(LoginModel model) {
                ...
                if(errorCode.equals("0")){
                    ...
                    loginServer2(context,User,Pwd,userCID);
//                    loginServer(User);//@@6.16
     ...
     * @param userId
     */
    public void loginServer2(final Context mcontext, final String userId, final String pwd, String cid){
        this.context=mcontext;//@@5.5
        Observable<LoginModel> observable = apiStores1.login2(userId,pwd,cid);//@@5.27添加app编号
        addSubscription(observable,new SubscriberCallBack<>(new ApiCallback<LoginModel>() {
            @Override
            public void onSuccess(LoginModel model) {
                int errorCode = model.getErrorCode();
                if(errorCode==0){
                    ...
                    SharedPreferencesManager.getInstance().putData(context,
                            SharedPreferencesManager.SP_FILE_GWELL,
                            "UserName",model.getName());//@@7.13 存储用户名
                    //跳转到主界面
                    mvpView.getDataSuccess(model);
                }else{
                    //跳转到登陆界面
//                    mvpView.getDataFail("登录失败，请重新登录");
                    mvpView.getDataFail(model.getError());//@@7.7
                }
            }
            @Override
            public void onFailure(int code, String msg) {
