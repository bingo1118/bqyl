...
//        mvpPresenter.getElectricHistoryInfo(userID, privilege + "", electricMac, electricType, electricNum, false);
    ...
    private void setPointsValues(List<TemperatureTime.ElectricBean> list) {
        ...
        for (int i = 0; i < maxNumberOfLines; ++i) {
            for (int j = 0; j < list.size(); ++j) {
                if (j >= 0 && j < 7) {
                    String str = list.get(j).getElectricValue();
//                    if (electricType.equals("7")) {
//                        data.put(j, str);
//                    }
                    float f = new BigDecimal(str).floatValue();
                    randomNumbersTab[i][j+1] = (f);
                }
            }
        }
    }
...
//    }
    ...
    private void resetViewport(List<TemperatureTime.ElectricBean> tem) {
        ...
        if(tem!=null&&tem.size()>0){
            value = (int) ((int)Utils.getMax(tem)*1.5);
        }
    ...
    public void getDataSuccess(List<TemperatureTime.ElectricBean> temperatureTimes) {
        if (temperatureTimes!=null&&temperatureTimes.size() >0) {
            electricBeen.clear();
...
//                    String str = data.get(i);
    ...
    @OnClick({R.id.btn_next, R.id.btn_before,R.id.btn_new,R.id.btn_turnto})
    public void onClick(View view) {
        switch (view.getId()) {
            ...
            case R.id.btn_new:
                ...
                mvpPresenter.getElectricTypeInfo(userID, privilege + "", electricMac, electricType, electricNum, page + "", false);
                break;
            case R.id.btn_turnto:
                try{
                    int turnto=Integer.parseInt(page_edit.getText().toString());
                    page = turnto;
                    page_edit.setText(page);
                    btnBefore.setClickable(false);
                    btnBefore.setBackgroundResource(R.mipmap.prve_an);
                    ...
                    T.showShort(context,"输入有误，请重新输入");
                    e.printStackTrace();
                }
                break;
        }
